...
import com.yyy.yongli.main.HaiHongMainActivity;
...
//        NetRxUtil.doRequestByRetrofit(NetRequestBody.getMultipartBody(getParams()));
    ...
    private void getContact() {
        ...
        new NetUtil(getParams(), url, new ResponseListener() {
            ...
            public void onSuccess(String string) {
                try {
                    initData(StringUtil.getHaiHongData(string));
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    private List<NetParams> getParams() {
        ...
        list.add(new NetParams("db", "HxHHWk2018New"));
    ...
    private void initData(String response) throws Exception {
        ...
        if (model.isSuccess()) {
            ...
            Intent intent = new Intent();
            intent.setClass(Login2Activity.this, HaiHongMainActivity.class);
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    LoadingDialog.cancelDialogForLoading();
    ...
    private void permission(Intent intent) {
        requestRunPermisssion(new String[]{Manifest.permission.CAMERA, Manifest.permission.READ_EXTERNAL_STORAGE}, new PermissionListener() {
            ...
            public void onGranted() {
                intent.setClass(Login2Activity.this, CaptureActivity.class);
                /* ZxingConfig是配置类
                 * 可以设置是否显示底部布局，闪光灯，相册，
                 * 是否播放提示音  震动
                 * 设置扫描框颜色等
...
//                Toast.makeText(LoginActivity.this, "授权失败", Toast.LENGTH_LONG).show();
                Toasts.showShort(Login2Activity.this, "授权失败");
            }
        });
    }
